...
/*
 * junheejang: my scheduling class: FIFO
 *
 */

struct task_struct *p;

extern int mysched_usage ;//수정
extern struct sched_class* classpointer ; //수정
extern struct sched_class fair_sched_class;//수정
extern const struct sched_class idle_sched_class;//수정
extern const struct sched_class rt_sched_class;//수정
extern const struct sched_class stop_sched_class;//수정
...
static int __init init_mysched(void)
{
	...
	classpointer = &my_sched_class;//수정

	fair_sched_class.next = &my_sched_class;//수정
...
static void __exit exit_mysched(void)
{
	...
	classpointer = NULL;//수정
	
	fair_sched_class.next = &idle_sched_class ;//수정

	return;

}

MODULE_AUTHOR("junheejang");
MODULE_DESCRIPTION("jun's Scheduler");
MODULE_LICENSE("GPL");
